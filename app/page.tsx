    ...
    <main className="min-h-screen flex flex-col bg-[#FAFAFA]">
      ...
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="px-5 py-2 bg-[#20808D] hover:bg-[#1A6B77] text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
              Try Pro
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-3xl">
          {/* Perplexity Title */}
          <h1 className="text-6xl font-light text-gray-900 text-center mb-12 tracking-tight" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            perplexity
          </h1>

          {/* Search Box */}
          <form onSubmit={handleSearch} className="relative mb-10">
            <div className="relative bg-white rounded-xl border-2 border-gray-200 hover:border-gray-300 focus-within:border-[#20808D] transition-all shadow-lg">
              <div className="flex items-center gap-3 px-5 py-4">
                ...
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => setShowComingSoon(true)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Attach file">
                    <svg className="w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                    </svg>
                  </button>
                  <button type="button" onClick={() => setShowComingSoon(true)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Voice input">
                    <svg className="w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <button
                    type="submit"
                    className="p-2 bg-[#20808D] hover:bg-[#1A6B77] rounded-lg transition-colors ml-1"
                    title="Submit"
                  >
          ...
          <div className="space-y-5">
            <p className="text-sm font-medium text-gray-500 px-1">Try asking:</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {suggestions.map((suggestion, index) => (
                <button
                  key={index}
                  onClick={() => handleSuggestionClick(suggestion)}
                  className="p-4 text-left bg-white hover:bg-gray-50 border border-gray-200 hover:border-gray-300 rounded-xl transition-all group shadow-sm"
                >
                  <p className="text-sm text-gray-600 group-hover:text-gray-900 leading-relaxed">
                    {suggestion}
                  </p>
                </button>
              ))}
